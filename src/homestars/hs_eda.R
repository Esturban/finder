...
smb_op_tel <-
  tapply(!is.na(smbs_dev_operating$telephone),
         !is.na(smbs_dev_operating$telephone),
         length) %>% .[order(., decreasing = T)]
smb_op_em <-
  tapply(!is.na(smbs_dev_operating$email),
         !is.na(smbs_dev_operating$email),
         length) %>% .[order(., decreasing = T)]
...
a_categories <- apexchart() %>%
  ...
  ax_subtitle(text = "Source: Homestars")
...
a_em <- apexchart() %>%
  ax_series2(smb_op_em) %>%
  ax_chart(type = "donut") %>%
  ax_plotOptions(pie = pie_opts(donut = list(size = "50%", background = "#BABABA"))) %>%
  ax_labels2(names(smb_op_em) %>% unique(.)) %>%
  ax_title(text = "% Emails Found") %>%
  ax_legend(show = F) 

a_tel <- apexchart() %>%
  ax_series2(smb_op_tel) %>%
  ax_chart(type = "donut") %>%
  ax_plotOptions(pie = pie_opts(donut = list(size = "50%", background = "#BABABA"))) %>%
  ax_labels2(names(smb_op_tel) %>% unique(.)) %>%
  ax_title(text = "% Phone Numbers Found") %>%
  ax_legend(show = F) 
