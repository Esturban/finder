...
lapply(cities, function(x) {
  ...
  intermediate_city_categories <- city_cats %>% dplyr::filter(City == x)
...
readr::write_csv(cities_categories_validated,file = here::here("data","homestars","categories","all_cities_categories.csv"))
# head(city_cats)
