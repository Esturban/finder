...
# for (i in 1:10) {
#   # c247_on$path[sample.int(nrow(c247_on), 1)] -> url_select
#   "https://canada247.info/explore/canada/ontario/toronto_division/toronto/promark-tool-and-manufacturing-416-755-2234.html"-> url_select
#   print("Source URL")
#   print(url_select)
#   url_select %>% read_html -> page
#   print("Results")
#   page %>% html_nodes(".width60p a") %>% html_attr("href") %>% gsub("https://canada247.info/redirect.php\\?to=", "", .) %>%
#     URLdecode(.) %>% .[!grepl("facebook|twitter|instagram|linkedin", .)] %>%
#     .[grepl("http", .)] %>% print(.)
# }
start <- Sys.time()
